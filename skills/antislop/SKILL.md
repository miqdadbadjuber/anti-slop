...
1. **Declare the setup before doing anything.** Tell the user you will (a) download the skill(s) they choose into `skills/<name>/` subfolders next to this file, and (b) append an antislop pointer block at the end of the project's entry file. Get approval. Never modify the entry file silently.
